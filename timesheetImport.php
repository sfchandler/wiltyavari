...
if(isset($_POST['submit'])) {
    if (($_POST['departmentId'] <> 'None') && isset($_POST['departmentId'])) {
        ...
        //echo $clientId. ' '.$stateId.' '.$deptId.' '.$startDate.' '.$endDate;
                    ...
                    if ($isUploaded) {
                        ...
                        for ($row = 2; $row <= $total_rows; $row++) {
                            ...
                            foreach ($single_row as $key => $value) {
                                echo uniqid('imp').date("Y-m-d",strtotime($value[0])).$value[1].$value[2].$value[3].$value[4].$value[5].$value[6].$value[7].$value[8].'<br>';
...
<div id="main" role="main">
    ...
    <div id="content" class="container-body">
        ...
        <h1>Timesheet Import</h1>
        <form class="smart-form" action="<?php echo $_SERVER['PHP_SELF'];?>" method="post" enctype="multipart/form-data">
            <fieldset>
                <legend><h5>Please Correctly select department according to excel sheet(Only one department per file)</h5></legend>
                <div class="row">
                    ...
                    <section class="col col-3">
                        <label for="departmentId" class="form-control">
