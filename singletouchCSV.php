...
foreach($payrunData as $data){
    ...
    foreach($csvData as $rs) {
        ...
        $empArray[$rs['candidateId']]['Payroll number'] = 1;
        ...
        $empArray[$rs['candidateId']]['empid'] = $rs['candidateId'];
        $yearToDate = getYearToDateData($mysqli,$rs['candidateId'],$wkendDateStart,$wkendDateEnd);
...
$objPHPExcel->getActiveSheet()->setCellValue('AA1', 'EMPID');
...
foreach ($empArray as $data) {
    ...
    $objPHPExcel->getActiveSheet()->setCellValue('AA'.$rowCount, $data['empid']);
}

$objWriter = PHPExcel_IOFactory::createWriter($objPHPExcel, 'CSV');
$filePath = './singletouch/singletouchFirst'.time().'.csv';
