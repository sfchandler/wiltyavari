...
.subheading{
  ...
  Date &nbsp; ' . date('d/m/Y') . ' </td>
    ...
    generateEmployeePAYGPDF($mysqli,$key,getEmployeeEmail($mysqli,$key),$html,$fromDate,$toDate);

    $payee = new Payee('DINB', 'O', ' ', ' ');//lump T fbt N
...
$empdupeFilePath = generateEmpdupe($mysqli,$toDate,$fromDate,$toDate,$financialYear,$payerIdentity,$supplier,$payeeArray,$fileExtType);
...
$pdfAll->SetAuthor(' ');
$pdfAll->SetTitle('PAYG Payment Summary');
$pdfAll->SetSubject('PAYG Payment Summary');
$pdfAll->SetKeywords('PAYG Payment Summary');
$pdfAll->setHeaderFont(Array(PDF_FONT_NAME_MAIN, '', 8));
$pdfAll->SetDefaultMonospacedFont(PDF_FONT_MONOSPACED);
define('PDF_CUSTOM_HEADER_STRING', ' PAYG Payment Summary     From:' . $fromDate . ' to:' . $toDate . '      User:' . $_SESSION['userSession'] . '    Printed: ' . date("Y-m-d H:i:s"));
...
$pdfAll->AddPage();
$fileName = 'paygSummaryReport_'.$fromDate.'to'.$toDate. time();
$filePathPDF = './payg/' . $fileName . '.pdf';
$pdfAll->writeHTML($htmlData, true, false, false, false, '');
$pdfAll->lastPage();
$pdfAll->Output(__DIR__ . '/payg/' . $fileName . '.pdf', 'F');
//generateEmployeePAYGPDF($mysqli,$empId,$email,$employeePAYGSummary,$fromDate,$toDate);

echo $filePathPDF.'^'.$empdupeFilePath;

function generateEmpdupe($mysqli,$currentDate,$fromDate,$toDate,$financialYear,$payerIdentity,$supplier,$payeeArray,$fileExtType){
    ...
    if($remainder == 0) {
        ...
        updatePAYGEMPDUPEFileList($mysqli,$empdupeFile);
        return $empdupeFile;
    }else{
...
function generateEmployeePAYGPDF($mysqli,$empId,$email,$employeePAYGSummary,$fromDate,$toDate){
    $pdf = new TCPDF(PDF_PAGE_ORIENTATION, PDF_UNIT, PDF_PAGE_FORMAT, true, 'UTF-8', false);
    $pdf->setHeaderTemplateAutoreset(true);
    $pdf->SetCreator(PDF_CREATOR);
    $pdf->SetAuthor(' ');
    $pdf->SetTitle('PAYG Summary');
    $pdf->SetSubject('PAYG Summary');
    $pdf->SetKeywords('PAYG Summary');
    $pdf->setHeaderFont(Array(PDF_FONT_NAME_MAIN, '', 8));
    $pdf->SetDefaultMonospacedFont(PDF_FONT_MONOSPACED);
    define('PDF_CUSTOM_HEADER_STRING', ' PAYG Summary     From:' . $fromDate . ' to:' . $toDate . '        ');
    ...
    $fileName = $empId.'_paygSummary_'.$fromDate.'_to_'.$toDate.'_'.date('Y-m-d');
    if (!file_exists('./documents/' . $empId)) {
        mkdir('./documents/' . $empId, 0777);
    }
    $filePathPDF = './documents/'.$empId.'/'. $fileName . '.pdf';
    $pdf->writeHTML($employeePAYGSummary, true, false, false, false, '');
    $pdf->lastPage();
    $pdf->Output(__DIR__ . '/documents/'.$empId.'/'. $fileName . '.pdf', 'F');
    updateCandidateDocs($mysqli,$empId,49,$fileName,$filePathPDF,$fromDate,$toDate,$toDate,'PAYG Summary');
    updatePAYGEmailList($mysqli,$empId,$email, $filePathPDF);
}
