...
$selectedYear = $_POST['rateYear'];
try {
    $timeSheetData = getTimeSheetTotalsForYearlyReport($mysqli, $weekendingDate, $empId, $jbCode, $profitCentre, $clCode);
    //echo var_dump($timeSheetData);
}catch (Exception $e){
    echo $e->getMessage();
}
...
<table border="1">
    ...
    if (!empty($timeSheetData)) {
        ...
        foreach ($timeSheetData as $data) {
            ...
            if (($data['ordTotal']) > 0) {
                ...
                $dayPayRate = getPayRateByYear($mysqli, $data['clientId'], $data['positionId'], $dayPayCatCode,$data['jobCode'],$selectedYear);
                $dayPayTotal = calculatePayAmount($dayTotal, $dayPayRate);
                $dayChargeRate = getChargeRateByYear($mysqli, $data['clientId'], $data['positionId'], $dayPayCatCode,$data['jobCode'],$selectedYear);
            ...
            if (($data['emgTotal']) > 0) {
                ...
                $emgPayRate = getPayRateByYear($mysqli, $data['clientId'], $data['positionId'], $emgPayCatCode,$data['jobCode'],$selectedYear);
                $emgPayTotal = calculatePayAmount($emgTotal, $emgPayRate);

                $emgChargeRate = getChargeRateByYear($mysqli, $data['clientId'], $data['positionId'], $emgPayCatCode,$data['jobCode'],$selectedYear);
            ...
            if (($data['aftTotal']) > 0) {
                ...
                $aftPayRate = getPayRateByYear($mysqli, $data['clientId'], $data['positionId'], $aftPayCatCode,$data['jobCode'],$selectedYear);
                $aftPayTotal = calculatePayAmount($aftTotal, $aftPayRate);

                $aftChargeRate = getChargeRateByYear($mysqli, $data['clientId'], $data['positionId'], $aftPayCatCode,$data['jobCode'],$selectedYear);
            ...
            if (($data['nightTotal']) > 0) {
                ...
                $nightPayRate = getPayRateByYear($mysqli, $data['clientId'], $data['positionId'], $nightPayCatCode,$data['jobCode'],$selectedYear);
                $nightPayTotal = calculatePayAmount($nightTotal, $nightPayRate);

                $nightChargeRate = getChargeRateByYear($mysqli, $data['clientId'], $data['positionId'], $nightPayCatCode,$data['jobCode'],$selectedYear);
            ...
            if (($data['rdoTotal']) > 0) {
                ...
                $rdoPayRate = getPayRateByYear($mysqli, $data['clientId'], $data['positionId'], $rdoPayCatCode,$data['jobCode'],$selectedYear);
                $rdoPayTotal = calculatePayAmount($rdoTotal, $rdoPayRate);

                $rdoChargeRate = getChargeRateByYear($mysqli, $data['clientId'], $data['positionId'], $rdoPayCatCode,$data['jobCode'],$selectedYear);
            ...
            if($data['clientId'] == 82){
                if(($data['hol_total']) > 0){
                    ...
                    $hld2PayRate = getPayRateByYear($mysqli, $data['clientId'], $data['positionId'], $holiday2PayCatCode,$data['jobCode'],$selectedYear);
                    $hld2PayTotal = calculatePayAmount($hld_total, $hld2PayRate);

                    $hld2ChargeRate = getChargeRateByYear($mysqli, $data['clientId'], $data['positionId'], $holiday2PayCatCode,$data['jobCode'],$selectedYear);
                ...
                if(($data['hldTotal']) > 0){
                    ...
                    $hldPayRate = getPayRateByYear($mysqli, $data['clientId'], $data['positionId'], $holidayPayCatCode,$data['jobCode'],$selectedYear);
                    $hldPayTotal = calculatePayAmount($hldTotal, $hldPayRate);
                    $hldChargeRate = getChargeRateByYear($mysqli, $data['clientId'], $data['positionId'], $holidayPayCatCode,$data['jobCode'],$selectedYear);
                ...
                if (($data['hldTotal']) > 0) {
                    ...
                    if (($wkTotal + $hldTotal) > $avgNormalHrs) {
                        ...
                        $hldPayRate = getPayRateByYear($mysqli, $data['clientId'], $data['positionId'], $hldSuperPayCatCode,$data['jobCode'],$selectedYear);
                        $hldPayTotal = calculatePayAmount($hldTotal, $hldPayRate);

                        $hldChargeRate = getChargeRateByYear($mysqli, $data['clientId'], $data['positionId'], $hldSuperPayCatCode,$data['jobCode'],$selectedYear);
                        $hldChargeTotal = calculateChargeAmount($hldTotal, $hldChargeRate);
            ...
            if (($data['satTotal']) > 0) {
                ...
                if (($wkTotal + $satTotal) <= $avgNormalHrs) {
                    ...
                    $satPayRate = getPayRateByYear($mysqli, $data['clientId'], $data['positionId'], $satSuperPayCatCode,$data['jobCode'],$selectedYear);
                    $satPayTotal = calculatePayAmount($satTotal, $satPayRate);

                    $satChargeRate = getChargeRateByYear($mysqli, $data['clientId'], $data['positionId'], $satSuperPayCatCode,$data['jobCode'],$selectedYear);
                    ...
                    $satPayRate = getPayRateByYear($mysqli, $data['clientId'], $data['positionId'], $satPayCatCode,$data['jobCode'],$selectedYear);
                    $satPayTotal = calculatePayAmount($satTotal, $satPayRate);
                    $satChargeRate = getChargeRateByYear($mysqli, $data['clientId'], $data['positionId'], $satPayCatCode,$data['jobCode'],$selectedYear);
                    $satChargeTotal = calculateChargeAmount($satTotal, $satChargeRate);
            ...
            if (($data['sunTotal']) > 0) {
                ...
                if (($wkTotal + $sunTotal) <= $avgNormalHrs) {
                    ...
                    $sunPayRate = getPayRateByYear($mysqli, $data['clientId'], $data['positionId'], $sunSuperPayCatCode,$data['jobCode'],$selectedYear);
                    $sunPayTotal = calculatePayAmount($sunTotal, $sunPayRate);
                    $sunChargeRate = getChargeRateByYear($mysqli, $data['clientId'], $data['positionId'], $sunSuperPayCatCode,$data['jobCode'],$selectedYear);
                    ...
                    $sunPayRate = getPayRateByYear($mysqli, $data['clientId'], $data['positionId'], $sunPayCatCode,$data['jobCode'],$selectedYear);
                    $sunPayTotal = calculatePayAmount($sunTotal, $sunPayRate);

                    $sunChargeRate = getChargeRateByYear($mysqli, $data['clientId'], $data['positionId'], $sunPayCatCode,$data['jobCode'],$selectedYear);
                    $sunChargeTotal = calculateChargeAmount($sunTotal, $sunChargeRate);
            ...
            if (($data['ovtTotal'])> 0) {
                ...
                $ovtPayRate = getPayRateByYear($mysqli, $data['clientId'], $data['positionId'], $overtimePayCatCode,$data['jobCode'],$selectedYear);
                $ovtPayTotal = calculatePayAmount($ovtTotal, $ovtPayRate);

                $ovtChargeRate = getChargeRateByYear($mysqli, $data['clientId'], $data['positionId'], $overtimePayCatCode,$data['jobCode'],$selectedYear);
            ...
            if (($data['satovtTotal']) > 0) {
                ...
                $satOvtPayRate = getPayRateByYear($mysqli, $data['clientId'], $data['positionId'], $satOvertimePayCatCode,$data['jobCode'],$selectedYear);
                $satOvtPayTotal = calculatePayAmount($satovtTotal, $satOvtPayRate);

                $satOvtChargeRate = getChargeRateByYear($mysqli, $data['clientId'], $data['positionId'], $satOvertimePayCatCode,$data['jobCode'],$selectedYear);
            ...
            if (($data['sunovtTotal']) > 0) {
                ...
                $sunPayRate = getPayRateByYear($mysqli, $data['clientId'], $data['positionId'], $sunOvertimePayCatCode,$data['jobCode'],$selectedYear);
                $sunPayTotal = calculatePayAmount($sunovtTotal, $sunPayRate);
                $chargeRate = getChargeRateByYear($mysqli, $data['clientId'], $data['positionId'], $sunOvertimePayCatCode,$data['jobCode'],$selectedYear);
            ...
            if (($data['povtTotal']) > 0) {
                ...
                $periodOvtPayRate = getPayRateByYear($mysqli, $data['clientId'], $data['positionId'], $periodOvertimePayCatCode,$data['jobCode'],$selectedYear);
                $periodOvtPayTotal = calculatePayAmount($povtTotal, $periodOvtPayRate);

                $periodOvtChargeRate = getChargeRateByYear($mysqli, $data['clientId'], $data['positionId'], $periodOvertimePayCatCode,$data['jobCode'],$selectedYear);
            ...
            if (($data['dblTotal']) > 0) {
                ...
                $dblPayRate = getPayRateByYear($mysqli, $data['clientId'], $data['positionId'], $doubletimePayCatCode,$data['jobCode'],$selectedYear);
                $dblPayTotal = calculatePayAmount($dblTotal, $dblPayRate);

                $dblChargeRate = getChargeRateByYear($mysqli, $data['clientId'], $data['positionId'], $doubletimePayCatCode,$data['jobCode'],$selectedYear);
    ...
    $fileName = 'timesheetAuditReport_selected_year_' . time();
