...
try {
    $dataArray = getConfirmedShiftsNotCheckedOutForReminder($mysqli);
    if(!empty($dataArray)) {
        foreach ($dataArray as $key => $value) {
            ...
            $fiveMinutesBefore = Carbon::createFromFormat('Y-m-d H:i', $shiftDate . ' ' . $shiftEnd)->subMinutes(5);
            $rosterEnd = Carbon::createFromFormat('Y-m-d H:i', $shiftDate . ' ' . $shiftEnd);
            if ($currentDateTime->between($fiveMinutesBefore, $rosterEnd)) {
                ...
                $title = 'Reminder Please ClockOut';
                $body = $client.' - '.$shiftEnd;
