...
$weekendingStartDate = $_POST['weekEndingStartDate'];
$weekendingFinishDate = $_POST['weekEndingFinishDate'];
...
$timeSheetData = getTimeSheetTotalsForAuditRangeReport($mysqli,$weekendingStartDate,$weekendingFinishDate,$empId,$jbCode,$profitCentre,$clientId);
...
if(!empty($timeSheetData)) {
    ...
    $pdf->SetTitle('TimeSheet Audit Range Report');
    $pdf->SetSubject(DOMAIN_NAME.' TimeSheet Audit range Report');
    $pdf->SetKeywords(DOMAIN_NAME.' TimeSheet Audit range Report');
    define('PDF_CUSTOM_HEADER_STRING', 'Week Ending Worked:' . $weekendingStartDate . ' to '.$weekendingFinishDate.'                          User:' . $_SESSION['userSession'] . '               Printed: ' . date("Y-m-d H:i:s"));
...
<table border="1">
    ...
    if (!empty($timeSheetData)) {
        ...
        foreach ($timeSheetData as $data) {
            ...
            if($candidateId != $data['candidateId']) {
                ...
                $allTotalChargeAmount = 0;
                $allTotalEMGPayAmount = 0;
                $allTotalEMGChargeAmount = 0;
                $allTotalDayPayAmount = 0;
                $allTotalDayChargeAmount = 0;
                $allTotalAftPayAmount = 0;
                $allTotalAftChargeAmount = 0;
                $allTotalNightPayAmount = 0;
                $allTotalNightChargeAmount = 0;
                $allTotalRDOPayAmount = 0;
                $allTotalRDOChargeAmount = 0;
                $allTotalHolidayPayAmount = 0;
                $allTotalHolidayChargeAmount = 0;
                $allTotalHolidayPayAmountWithSuper = 0;
                $allTotalHolidayChargeAmountWithSuper = 0;
                $allTotalSatPayAmount = 0;
                $allTotalSatChargeAmount = 0;
                $allTotalSatPayAmountWithSuper = 0;
                $allTotalSatChargeAmountWithSuper = 0;
                $allTotalSunPayAmount = 0;
                $allTotalSunChargeAmount = 0;
                $allTotalSunPayAmountWithSuper = 0;
                $allTotalSunChargeAmountWithSuper = 0;
                $allTotalOvertimePayAmount = 0;
                $allTotalOvertimeChargeAmount = 0;
                $allTotalSatOvertimePayAmount = 0;
                $allTotalSatOvertimeChargeAmount = 0;
                $allTotalSunOvertimePayAmount = 0;
                $allTotalSunOvertimeChargeAmount = 0;
                $allTotalPeriodOvertimePayAmount = 0;
                $allTotalPeriodOvertimeChargeAmount = 0;
                $allTotalDoubletimePayAmount = 0;
                $allTotalDoubletimeChargeAmount = 0;*/
                ...
                $html = $html . '<tr class="zebra' . ($i++ & 1) . '"><td colspan="11" class="title">' . $employeeName .' '.$data['jobcode'].' '.$position.'</td></tr>';
            ...
            if (($data['ordTotal']) > 0) {
                ...
                $dayPayRate = getPayRate($mysqli, $data['clientId'], $data['positionId'], $dayPayCatCode);
                ...
                $html = $html . '<td class="cellWidth">' . $weekendingStartDate.' to '.$weekendingFinishDate . '</td>';
            ...
            if (($data['emgTotal']) > 0) {
                ...
                $emgPayRate = getPayRate($mysqli, $data['clientId'], $data['positionId'], $emgPayCatCode);
            ...
            if (($data['aftTotal']) > 0) {
                ...
                $aftPayRate = getPayRate($mysqli, $data['clientId'], $data['positionId'], $aftPayCatCode);
            ...
            if (($data['nightTotal']) > 0) {
                ...
                $nightPayRate = getPayRate($mysqli, $data['clientId'], $data['positionId'], $nightPayCatCode);
            ...
            if (($data['rdoTotal']) > 0) {
                ...
                $rdoPayRate = getPayRate($mysqli, $data['clientId'], $data['positionId'], $rdoPayCatCode);
            ...
            if (($data['hldTotal']) > 0) {
                ...
                if (($wkTotal + $hldTotal) > $avgNormalHrs) {
                    ...
                    $hldPayRate = getPayRate($mysqli, $data['clientId'], $data['positionId'], $hldSuperPayCatCode);
                    $hldPayTotal = calculatePayAmount($hldTotal, $hldPayRate);

                    $hldChargeRate = getChargeRate($mysqli, $data['clientId'], $data['positionId'], $hldSuperPayCatCode);
                    ...
                    $hldPayRate = getPayRate($mysqli, $data['clientId'], $data['positionId'], $holidayPayCatCode);
                    $hldPayTotal = calculatePayAmount($hldTotal, $hldPayRate);
                    $hldChargeRate = getChargeRate($mysqli, $data['clientId'], $data['positionId'], $holidayPayCatCode);
                    $hldChargeTotal = calculateChargeAmount($hldTotal, $hldChargeRate);
            ...
            if (($data['satTotal']) > 0) {
                ...
                if (($wkTotal + $satTotal) <= $avgNormalHrs) {
                    ...
                    $satPayRate = getPayRate($mysqli, $data['clientId'], $data['positionId'], $satSuperPayCatCode);
                    $satPayTotal = calculatePayAmount($satTotal, $satPayRate);

                    $satChargeRate = getChargeRate($mysqli, $data['clientId'], $data['positionId'], $satSuperPayCatCode);
                    ...
                    $satPayRate = getPayRate($mysqli, $data['clientId'], $data['positionId'], $satPayCatCode);
                    $satPayTotal = calculatePayAmount($satTotal, $satPayRate);
            ...
            if (($data['sunTotal']) > 0) {
                ...
                if (($wkTotal + $sunTotal) <= $avgNormalHrs) {
                    ...
                    $sunPayRate = getPayRate($mysqli, $data['clientId'], $data['positionId'], $sunSuperPayCatCode);
                    $sunPayTotal = calculatePayAmount($sunTotal, $sunPayRate);
                    $sunChargeRate = getChargeRate($mysqli, $data['clientId'], $data['positionId'], $sunSuperPayCatCode);
                    ...
                    $sunPayRate = getPayRate($mysqli, $data['clientId'], $data['positionId'], $sunPayCatCode);
                    $sunPayTotal = calculatePayAmount($sunTotal, $sunPayRate);
            ...
            if (($data['ovtTotal'])> 0) {
                ...
                $ovtPayRate = getPayRate($mysqli, $data['clientId'], $data['positionId'], $overtimePayCatCode);
                $ovtPayTotal = calculatePayAmount($ovtTotal, $ovtPayRate);

                $ovtChargeRate = getChargeRate($mysqli, $data['clientId'], $data['positionId'], $overtimePayCatCode);
            ...
            if (($data['satovtTotal']) > 0) {
                ...
                $satOvtPayRate = getPayRate($mysqli, $data['clientId'], $data['positionId'], $satOvertimePayCatCode);
                $satOvtPayTotal = calculatePayAmount($satovtTotal, $satOvtPayRate);

                $satOvtChargeRate = getChargeRate($mysqli, $data['clientId'], $data['positionId'], $satOvertimePayCatCode);
            ...
            if (($data['sunovtTotal']) > 0) {
                ...
                $sunPayRate = getPayRate($mysqli, $data['clientId'], $data['positionId'], $sunOvertimePayCatCode);
                $sunPayTotal = calculatePayAmount($sunovtTotal, $sunPayRate);
                $chargeRate = getChargeRate($mysqli, $data['clientId'], $data['positionId'], $sunOvertimePayCatCode);
            ...
            if (($data['povtTotal']) > 0) {
                ...
                $periodOvtPayRate = getPayRate($mysqli, $data['clientId'], $data['positionId'], $periodOvertimePayCatCode);
                $periodOvtPayTotal = calculatePayAmount($povtTotal, $periodOvtPayRate);

                $periodOvtChargeRate = getChargeRate($mysqli, $data['clientId'], $data['positionId'], $periodOvertimePayCatCode);
            ...
            if (($data['dblTotal']) > 0) {
                ...
                $dblPayRate = getPayRate($mysqli, $data['clientId'], $data['positionId'], $doubletimePayCatCode);
                $dblPayTotal = calculatePayAmount($dblTotal, $dblPayRate);

                $dblChargeRate = getChargeRate($mysqli, $data['clientId'], $data['positionId'], $doubletimePayCatCode);
            ...
            if ($client == $data['clientId']) {
                ...
                foreach ($clientArray as $key => $value) {
                    if ($key == $client) {
                        $clientArray[$key]['payUnits'] = $value['payUnits'] + $subTotalUnits;
                        $clientArray[$key]['payAmount'] = round(($value['payAmount'] + $totalPayAmount),2);
                        $clientArray[$key]['billUnits'] = $value['billUnits'] + $subTotalUnits;
                        $clientArray[$key]['billAmount'] = round(($value['billAmount'] + $totalChargeAmount),2);
                        $clientArray[$key]['superTypeCount'] = $value['superTypeCount'] + $allSuperTypeCount;
                        $clientArray[$key]['superPayAmount'] = round(($value['superPayAmount'] + $superPayAmount),2);
                        $clientArray[$key]['tax'] = $value['tax'] + $paygTax;
                        $clientArray[$key]['net'] = $value['net'] + $net;
                        $clientArray[$key]['gross'] = $value['gross'] + $allGrossPerClient;
                    }
                }
            }else{
    ...
    define('PDF_CUSTOM_HEADER_STRING1', 'Week Ending Worked:' . $weekendingStartDate.' to '.$weekendingFinishDate . '   User:' . $_SESSION['userSession'] . '   Printed: ' . date("Y-m-d H:i:s"));
...
td.sumAmount{
    ...
    foreach ($clientArray as $key => $value) {
        ...
        $clientSum = $clientSum . '<tr class="zebra' . ($i++ & 1) . '"><td class="tscode"></td><td class="sumDesc">Hourly</td><td class="sumUnit">' . number_format($clientArray[$key]['payUnits'], 2) . '</td><td class="sumAmount">' . number_format($clientArray[$key]['payAmount'], 2) . '</td><td class="sumUnit">' . number_format($clientArray[$key]['billUnits'], 2) . '</td><td class="sumAmount">' . number_format($clientArray[$key]['billAmount'],2) . '</td></tr>';
        $clientSum = $clientSum . '<tr class="zebra' . ($i++ & 1) . '"><td class="tscode"></td><td class="sumDesc">Tax</td><td class="sumUnit">0.00</td><td class="sumAmount">' . number_format($clientArray[$key]['tax'], 2) . '</td><td class="sumUnit"></td><td class="sumAmount"></td></tr>';
        $clientSum = $clientSum . '<tr class="zebra' . ($i++ & 1) . '"><td class="tscode"></td><td class="sumDesc">Net</td><td class="sumUnit">0.00</td><td class="sumAmount">' . number_format($clientArray[$key]['net'], 2) . '</td><td class="sumUnit"></td><td class="sumAmount"></td></tr>';
        $clientSum = $clientSum . '<tr class="zebra' . ($i++ & 1) . '"><td class="tscode"></td><td class="sumDesc">Gross</td><td class="sumUnit"></td><td class="sumAmount">' . number_format($clientArray[$key]['gross'], 2) . '</td><td class="sumUnit"></td><td class="sumAmount"></td></tr>';

        $clientSum = $clientSum . '<tr class="zebra' . ($i++ & 1) . '"><td class="tscode"></td><td class="sumDesc">SuperAnnuation</td><td class="sumUnit">' . number_format($clientArray[$key]['superTypeCount'], 2) . '</td><td class="sumAmount">' . number_format($clientArray[$key]['superPayAmount'], 2) . '</td><td class="sumUnit"></td><td class="sumAmount"></td></tr>';
