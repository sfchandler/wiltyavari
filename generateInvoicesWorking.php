    ...
    if(!checkInvoiceGeneration($mysqli,$weekendingDate)) {
        ...
        class INVOICEPDF extends TCPDF
        {
            ...
            public function Footer()
            {
                ...
                $this->SetY(-11);
                $this->Cell(0, 10,'                   Payment Preferred by EFT: Bank Name :   BSB:   Account Number :  ', 0, false, 'L', 0, '', 0, false, 'T', 'M');
                $this->SetY(-7);
            ...
            foreach ($invData as $data) {
                ...
                if($invType == 'Each Employee'){
                    ...
                    if ($jbCode != $data['jobcode']) {
                        $html = $html . '<tr><td colspan="5"></td></tr>';
                        //$html = $html . '<tr><td colspan="3" class="lblBold">JobCode:' . $data['jobcode'] . ' - ' . getClientNameByClientId($mysqli, $data['clientId']) . ', ' . getPositionByPositionId($mysqli, $data['positionId']) . ' - ' . $weekendingDate . '<br></td></tr>';//date('Y-m-d')
                        $jbCode = $data['jobcode'];
                    }
