    ...
    if(is_string($bulkCanId)){
        ...
        foreach ($bulkCandidates as $emp){
            foreach($period as $key=>$value){
                ...
                $shiftStatus = saveAndDisplayReleaseShift($mysqli, $shDate, $shDay, $clid, $stid, $did, $emp, $StartTime, $EndTime, $workBreak, $note, $shiftCopy, $startDate, $endDate, $dateRange, $positionId, $shStatus, $addressId,$consultantId,$_SESSION['userSession']);
            }
        }
    ...
    foreach($lastWeekRoster as $data) {
        foreach ($daterange as $date) {
            if($data['shiftDay'] == dayOfWeek($date->format('Y-m-d'))){
                echo saveAndDisplayReleaseShift($mysqli,$date->format('Y-m-d'),$data['shiftDay'],$clientId,$stateId,$deptId,$data['candidateId'],$data['shiftStart'],$data['shiftEnd'],$data['workBreak'],'','','','',$dateRange,$positionId,'OPEN',$data['addressId'],$consultantId,$_SESSION['userSession']);
            }
        }
    }
}else {
    $shiftStatus = saveAndDisplayReleaseShift($mysqli, $shDate, $shDay, $clid, $stid, $did, $canid, $StartTime, $EndTime, $workBreak, $note, $shiftCopy, $startDate, $endDate, $dateRange, $positionId, $shStatus, $addressId,$consultantId,$_SESSION['userSession']);
    echo $shiftStatus;
}
?>