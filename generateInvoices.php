...
error_reporting(E_ALL);
    ...
    if(!checkInvoiceGeneration($mysqli,$weekendingDate)) {
        ...
        class INVOICEPDF extends TCPDF
        {
            ...
            {
                $this->companyEFTNotes =  'Bank Name :   BSB:    Account Number :  ';
            }
            /**
             * @return mixed
             */
            ...
            public function Header()
            {
                ...
                $this->Cell(0, '', DOMAIN_NAME.' ', 0, $ln = 0, 'C', 0, '', 0, false, 'B', 'B');
                $this->Ln(4);
                $this->SetFont('helvetica', 'B', 7);
        ...
        if (!empty($invData) || ($invData != '')) {
            ...
            $html = $html . '<style>.caps{text-transform: uppercase}hr{ text-decoration-style: solid; width: 860px;}.lblAmount{font-weight: bold;font-size: 11pt}.lblBold{font-weight: bold;}.empName{font-style: italic;}</style><div>
    <br><br><br><table width="950px" nobr="true">
        <thead></thead>
        <tbody class="invBody">';
            ...
            foreach ($invData as $data) {
                ...
                if($invType == 'Each Employee'){
                    ...
                    if(!empty($ratesArray) && ($total != 0)) {
                        ...
                        saveInvoice($mysqli, $invoiceNo, date('Y-m-d', strtotime($invDate)), $weekendingDate, $client, round($total,2), round(($total * 10 / 100),2),round((($total * (10 / 100)) + $total),2));
                        //if ($i != $len - 1) {
            ...
            foreach ($invData as $eachJob) {
                ...
                if($invType == 'Each Job'){
                    ...
                    if ($jbCode != $eachJob['jobcode']) {
                        ...
                        $html = $html . '<tr nobr="true"><td colspan="5"><span style="font-size: 6pt">' . getCompanyNote($mysqli, $companyId) . '</span></td></tr>';
                        $html = $html . '<tr nobr="true"><td colspan="5"><hr></td></tr>';
                        //$html = $html . '<tr nobr="true"><td colspan="3" class="lblBold">JobCode:' . $eachJob['jobcode'] . ' - ' . getClientNameByClientId($mysqli, $eachJob['clientId']) . ', ' . getPositionByPositionId($mysqli, $eachJob['positionId']) . ' - ' . $weekendingDate . '<br></td></tr>';//date('Y-m-d')
                        $jbCode = $eachJob['jobcode'];
                        $html = $html . '<tr nobr="true"><td colspan="5" class="lblBold">JobCode:' . $eachJob['jobcode'] . ' - ' . getClientNameByClientId($mysqli, $eachJob['clientId']).' '. getDepartmentById($mysqli,getDepartmentIdByJobCode($mysqli,$eachJob['jobcode'])). ', ' . getPositionByPositionId($mysqli, $eachJob['positionId']) . ' - ' . $weekendingDate . '<br></td></tr>';//date('Y-m-d')
                        if($eachJob['clientId'] == 294){
                            $html = $html.' '. getDepartmentById($mysqli,getDepartmentIdByJobCode($mysqli,$eachJob['jobcode']));
                        }
                   ...
                   /* if ($canId != $eachJob['candidateId']) {
                        $canId = $eachJob['candidateId'];
                        $html = $html . '<tr nobr="true"><td class="empName">' . strtoupper(getCandidateFullName($mysqli, $eachJob['candidateId'])) . '</td><td></td><td></td></tr>';
                    }*/
            ...
            foreach ($invData as $allData) {
                ...
                if($invType == 'All Jobs'){
                    ...
                    if (($client != $allData['clientId']) && (!empty($client))) {
                        ...
                        $hld2ChargeRate = 0;
                        $rdoChargeRate = 0;
                        /*$html = $html . '<tr nobr="true"><td colspan=5><br/></td></tr>';
                        $html = $html . '<tr nobr="true"><td><div align="left" class="caps">Client Code:  <span class="lblBold">' . getClientCodeById($mysqli, $allData['clientId']) . '</span><br/>Invoice No:  <span class="lblBold">' . $invoiceNo . '</span><br/>' . getClientAccountManagerFromJobDetail($mysqli, $allData['clientId'],$allData['jobcode']) . '<br/>' . getClientNameByClientId($mysqli, $allData['clientId']) . '<br/>' . getClientAddress($mysqli, $allData['clientId']) . '</div></td><td align="left" style="font-size: 8pt; font-weight: bold ">Terms:' . $terms . '<br><span style="font-size: 8pt; font-weight: bold">DUE DATE<br/>' . date('d/m/Y', strtotime($invoiceDate . ' + ' . $termGap . ' days')) . '</span></td><td></td><td></td><td></td></tr>';
                        $html = $html . '<tr nobr="true"><td colspan="5"><br/><br/></td></tr>';
                        $html = $html . '<tr nobr="true"><td colspan="5"><span style="font-size: 6pt">If you have any queries regarding this invoice please contact CHANDLER RECRUITMENT ACCOUNTS on 03 9596 9777</span></td></tr>';
                        $html = $html . '<tr nobr="true"><td colspan="5"><hr></td></tr>';*/
                    ...
                    foreach ($invoiceAdditionsAll as $inAdd) {
                            $inAddAmount = $inAdd['amount'];
                            $html = $html . '<tr nobr="true"><td width="60%">' . $inAdd['description'] . '(' . $inAdd['units'] . ' Hours @ $' . $inAddAmount . ')</td><td width="10%" align="right">$' . number_format($inAddAmount, 2) . '</td><td width="10%"></td></tr>';
                            updateInvoiceAddition($mysqli, $inAdd['id'], $invoiceNo);
                            $total = $total + $inAddAmount;
                    }
                    $total = $total + ($emgTotal + $ordTotal + $aftTotal + $nightTotal + $satTotal + $sunTotal + $ovtTotal + $dblTotal + $hldTotal + $hld2Total + $rdoTotal);
