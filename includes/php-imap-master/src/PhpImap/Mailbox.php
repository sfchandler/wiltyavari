...
class Mailbox {
	...
	public function __construct($imapPath, $login, $password, $attachmentsDir = null, $serverEncoding = 'UTF-8') {
		...
		if($attachmentsDir) {
			...
			$this->attachmentsDir = rtrim(($attachmentsDir), '\\/');//rtrim(real_path($attachmentsDir), '\\/');
		}
	}
	...
	public function getMailsInfo(array $mailsIds) {
		...
		if(is_array($mails) && count($mails))
		{
			foreach($mails as &$mail)
			{
                ...
                if(isset($mail->uid)) {
                    $mail->uid = $this->decodeMimeStr($mail->uid, $this->serverEncoding);
                }
			}
		}
		return $mails;
	...
	protected function initMailPart(IncomingMail $mail, $partStructure, $partNum, $markAsSeen = true) {
		...
		if($attachmentId) {
			...
			$mailDate = date('Y-m-d', strtotime($mail->date));
            $alphabeticalDirectory = strtoupper($fileName[0]);
			// Adding a new filename if already exists
            /*if (file_exists($this->attachmentsDir.DIRECTORY_SEPARATOR.$mailDate.DIRECTORY_SEPARATOR.$alphabeticalDirectory.DIRECTORY_SEPARATOR.$fileName)) {
                $fileName = $mail->id.'-'.$fileName;
            }*/
            ...
            if ($this->attachmentsDir) {
                ...
                //$attPath = str_replace('/var/www/html/','./',$this->attachmentsDir).'/';
                //echo  'DIRECTORY PATH'.$this->attachmentsDir;
                if (!file_exists($this->attachmentsDir.DIRECTORY_SEPARATOR.$mailDate.DIRECTORY_SEPARATOR.$alphabeticalDirectory)) {
                        chown($this->attachmentsDir.DIRECTORY_SEPARATOR.$mailDate.DIRECTORY_SEPARATOR.$alphabeticalDirectory,'chandler');
                        mkdir($this->attachmentsDir.DIRECTORY_SEPARATOR.$mailDate.DIRECTORY_SEPARATOR.$alphabeticalDirectory , 0777,true);
                }
                $attachment->filePath = $this->attachmentsDir.DIRECTORY_SEPARATOR.$mailDate.DIRECTORY_SEPARATOR.$alphabeticalDirectory.DIRECTORY_SEPARATOR.$fileName;
                //str_replace('./','',
                ...
                /* Restricting file extensions of attachments to be retrieved */
                $extension = pathinfo($attachment->filePath, PATHINFO_EXTENSION);
                //$filenameString = 'resume';
                /*if (strpos(strtolower($attachment->name), strtolower($filenameString))) {*/
                    if ($extension == 'pdf' || $extension == 'doc' || $extension == 'docx' || $extension == 'rtf') {
                        if (file_exists($attachment->filePath) == FALSE) {
