...
require_once("./includes/pdf2text/pdf2textO.php");*/
...
class Filetotext {
    ...
    public function convertToText() {
		...
		if(isset($this->filename) && !file_exists($this->filename)) {
            ...
            if (($file_ext == "doc") || ($file_ext == "docx") || ($file_ext == "DOC") || ($file_ext == "DOCX")) {
                if (strtoupper($file_ext) == "DOC") {
                    return $this->read_doc();
                } else if (strtoupper($file_ext) == "DOCX") {
	...
	private function pdf2textConversion(){
		$result = pdf2text ($this->filename);
	...
	private function read_docx(){
        ...
        if((empty($striped_content)) || is_null($striped_content)){
            //return $this->docx2text($this->filename);
            return 'Cannot converted to Text';
        }else{
            return $striped_content;
        }
	}
...
//				if (!(empty($options["Length1"]) &&  empty($options["Subtype"])) )
		...
		$string = $this->getTextUsingTransformations($texts, $transformations);
		if($string==="" || is_null($string)){
			...
			try{
				...
				$this->decodedtext = nl2br($pdf->Text);//outPut($pdf->Text);
			}catch (Exception $e1) {	
				echo 'PDF CONVERSION ERROR: ',  $e1->getMessage(), "\n";
			}
		}else{
			$this->decodedtext = $string;
		}
