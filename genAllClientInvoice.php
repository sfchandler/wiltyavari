...
if(checkInvoiceGeneration($mysqli,$weekendingDate) == 'FALSE') {
    ...
    foreach ($allClients as $allClient) {
        ...
        <tbody class="invBody">';
            ...
            $fileName = 'invoice_' . $clientCode . '_' . $invoiceNo . '_' . date('Y-m-d') . '_' . time();
            ...
            try {
                $invoiceSave = saveInvoicePathByClient($mysqli, $weekendingDate, $clientCode, $downloadPath, $fileName . '.pdf',$invDate);
            }catch (Exception $e){
                echo $e->getMessage();
            }
        }
