...
$pdf->SetAuthor(' ');
$pdf->SetTitle('Payroll Calculation Report Preview');
$pdf->SetSubject('Payroll Calculation Report Preview');
$pdf->SetKeywords('Payroll Calculation Report Preview');
$pdf->setHeaderFont(Array(PDF_FONT_NAME_MAIN, '', 8));
$pdf->SetDefaultMonospacedFont(PDF_FONT_MONOSPACED);
define('PDF_CUSTOM_HEADER_STRING','Payrun No :     Week Ending Worked:'.$weekendingDate.'    User:'.$_SESSION['userSession'].'    Printed: '.date("Y-m-d H:i:s"));
...
$html = $html.'<div class="pageTitle">Payroll Calculation Report Preview</div><table border="1"><thead>
    <tr>
            ...
            if ($data['candidateId'] == $key) {
                ...
                foreach ($deductCodes as $deduct) {
                    ...
                    if($deduct['taxorder'] == 'before') {
                        ...
                        $taxOrder = 'after';
                        $html = $html.'-'.number_format($deduct['transCodeAmount'],2);
                        $candidateArray[$data['candidateId']]['afterTaxDeductTotal'] = $candidateArray[$data['candidateId']]['afterTaxDeductTotal'] + $deduct['transCodeAmount'];
                        $totalAfterDeductionTotal = $totalAfterDeductionTotal + $deduct['transCodeAmount'];
                ...
                if($allowanceAmount > 0){
                    ...
                    foreach ($tr_codes as $allowanceCode){
                        ...
                        if($allowanceCode['taxorder'] == 'after'){
                            ...
                            $candidateArray[$data['candidateId']]['afterTaxAllowanceTotal'] = $candidateArray[$data['candidateId']]['afterTaxAllowanceTotal'] + $allowanceAmount;
                        }
                    }
                    if($no_after_tax != 1) {
...
$fileName = 'preview_payrollCal'.time();
