$(document).ready(function (){
    ...
    $(document).on('click','#empPosition',function(){
            $('#employeeList').html('');
    ...
    $(document).on('click','#checkMale',function(){
        if($(this).prop('checked')==true){
            $('#checkboxes').find('.Male').prop('checked', true);
            $('#checkboxes').find('.Female').prop('checked', false);
    ...
    $(document).on('click','#checkFemale',function(){
        if($(this).prop('checked')==true){
            $('#checkboxes').find('.Male').prop('checked', false);
            $('#checkboxes').find('.Female').prop('checked', true);
            $('#checkall').prop('checked', false);
            $('#checkMale').prop('checked', false);
        }else if($(this).prop('checked')==false){
            $('#checkall').prop('checked', false);
    ...
    function displayAssignedEmployees(){
        ...
        $.ajax({
            url:"getAssignedEmployees.php",
            type:"POST",
            data : {client_id:client_id,state_id:state_id,dept_id:dept_id,position_id:position_id},
            dataType:"json",
            success: function(data){
                console.log('.....'+data);
                $html = '<b>Select Employees to release<div style="padding-bottom: 5px;">' +
                    '<input type="checkbox" id="checkall" checked/>' +
                    '<label for="checkall"><b>check/uncheck all</b></label>' +
                    '&nbsp;&nbsp;' +
                    '<input type="checkbox" id="checkMale"/>' +
                    '<label for="checkMale"><b>check/uncheck Male</b></label>' +
                    '&nbsp;&nbsp;' +
                    '<input type="checkbox" id="checkFemale"/>' +
                    '<label for="checkFemale"><b>check/uncheck Female</b></label>' +
                    '</div></b><div id="checkboxes">';
                $.each(data, function (index,obj){
                    $html = $html + '<div class="'+obj['gender']+'"><input type="checkbox" name="can_id[]" id="' + obj['candidate_id'] + '" value="' + obj['candidate_id'] + '" class="'+obj['gender']+'" checked/> '+obj['name']+'('+obj['candidate_id']+')'+obj['mobileNo']+'</div>';
                });
                $html = $html+'</div>';
                $('#employeeList').html('');
    ...
    function releaseTableInit(){
        ...
        $('#relTbl thead th.thFilter').each(function() {
    ...
    $(document).on('click','#releaseBtn', function (e){
        ...
        let checkbox_value = "";
        //$(":checkbox")
        $("input[name='can_id[]']").each(function () {
            let ischecked = $(this).is(":checked");
            if (ischecked) {
                checkbox_value += $(this).val() + ",";
            }
        });
        //console.log('CHKBOXVAL'+checkbox_value);
        $.ajaxSetup({
            headers: {
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                $.each(data, function (index,obj){
                    ...
                    $('#'+obj['p_id']).before('<img class="indicator" width="20" height="20" src="img/tick-box.gif" alt=""/>');
                    $('#'+obj['o_id']).before('<img class="indicator" width="20" height="20" src="img/cross-mark.png" alt=""/>');
                    if(obj['msg'] !== ''){
                        $('#msg').html(obj['msg']);
                    }
                });
    ...
    $(document).on('click','.releaseShiftRemoveBtn', function (){
        let rel_shift_id = $(this).closest('td').data('relshiftid');
        let action = 'REMOVE';
        $.ajax({
            url: "confirmReleaseShifts.php",
            ...
            success: function(data){
                console.log('>>>>>>>>>>>>>>>>>>>>>>>>>>>>'+data);
                if(data[0].status === 'shiftRemoved'){
                    $('.error').html('');
                    $('.error').html('Released Shift Removed from list');
                }else{
                    $('.error').html('');
                    $('.error').html(data[0].status);
                }
    ...
    $(document).on('click','#exportConReleaseBtn', function(){
         $.ajax({
            url: "confirmReleaseShiftExport.php",
            type:"POST",
            dataType:"text",
            success: function(data){
                if(data != ''){
