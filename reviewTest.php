...
if(empty($messageId)){
    ...
    echo '|||FIRST'.$firstName.'LAST'.$lastName.'EMAIL'.$emailAddress.'PHONE'.trim(str_replace(' ', '', $phoneNumber)).'||||<br>';
    ...
    $stmt->store_result();
    $stmt->fetch();
    echo $num_of_rows = $stmt->num_rows;
    if ($num_of_rows > 0) {
        echo 'ALL'.$num_of_rows.$matchPercentage = ((4 / 4) * 100) . '%';
    } else {
        echo 'EMPTY'.$num_of_rows;
    }
}
