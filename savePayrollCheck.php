...
try {
    if (!empty($_POST['shiftId'])) {
        ...
        foreach ($_POST['shiftId'] as $cnt => $shiftId) {
            ...
            $row = array('shiftId' => $_POST['shiftId'][$cnt], 'shiftDay' => $_POST['shiftDay'][$cnt], 'shiftDate' => $_POST['shiftDate'][$cnt], 'candidateId' => $_POST['candidateId'][$cnt], 'clientId' => $_POST['clid'][$cnt], 'positionId' => $_POST['posid'][$cnt], 'deptId' => $_POST['deptId'][$cnt], 'jobCode' => $_POST['jobCode'][$cnt], 'shiftStart' => $_POST['shiftStart'][$cnt], 'shiftEnd' => $_POST['shiftEnd'][$cnt], 'workBreak' => $_POST['workBreak'][$cnt], 'wrkhrs' => $_POST['wrkhrs'][$cnt], 'wkendingDate' => $_POST['wkendingDate']);
            $shiftArray[] = $row;
            if(!empty($_POST['wkendingDate'])) {
                $status = savePayrollCheck($mysqli, $_POST['shiftId'][$cnt], $_POST['shiftDay'][$cnt], $_POST['shiftDate'][$cnt], $_POST['candidateId'][$cnt], $_POST['clid'][$cnt], $_POST['posid'][$cnt], $_POST['deptId'][$cnt], $_POST['jobCode'][$cnt], $_POST['shiftStart'][$cnt], $_POST['shiftEnd'][$cnt], $_POST['workBreak'][$cnt], $_POST['wrkhrs'][$cnt], $_POST['wkendingDate'], $_POST['timeSheetTick'][$cnt]);
            }
        }
        if (!empty($shiftArray)) {
            $html = $html . '<style>
        ...
        <table border="1">
            ...
            $time = time();
            $fullPath = './timesheet/payroll_check_saving-' . $time . '.xlsx';
